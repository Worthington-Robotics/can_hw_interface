...
namespace robotmotors {
    ...
    bool TalonFxMotor::configure(rclcpp::Node & node, rclcpp::SubscriptionOptionsWithAllocator<std::allocator<void>> opts, std::string & topicStr, std::shared_ptr<std::map<std::string, double>> config) {
        topic = std::make_shared<std::string>(topicStr);

        demands = node.create_subscription<can_msgs::msg::MotorMsg>(topic->c_str(), rclcpp::SystemDefaultsQoS(), std::bind(&TalonFxMotor::setCallback, this, _1), opts);
