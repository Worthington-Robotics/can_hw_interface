...
namespace robotmotors {
    ...
    bool VictorSpxMotor::configure(rclcpp::Node & node, rclcpp::SubscriptionOptionsWithAllocator<std::allocator<void>> opts, std::string & topicStr, std::shared_ptr<std::map<std::string, double>> config) {
        ...
        demands = node.create_subscription<can_msgs::msg::MotorMsg>(topic->c_str(), rclcpp::SystemDefaultsQoS(), std::bind(&VictorSpxMotor::setCallback, this, _1), opts);
    ...
    void VictorSpxMotor::publishNewSensorData(){
